...
import { BsSearch, BsXCircle, BsCalendarPlusFill } from "react-icons/bs";
...
function TelaConsultas() {
    ...
    const handleBuscar = async () => {
        ...
        try {
            const dados = filtro
                ? await ConsultaService.filtrar(filtro)
        ...
        <Container fluid className="py-4 px-4">
            ...
            <div className="bg-white rounded-3 shadow-sm p-3 mb-4" style={{ border: '1px solid #e9ecef' }}>
                <InputGroup>
                    ...
                    <Button variant="primary" onClick={handleBuscar} style={{ borderRadius: '0 6px 6px 0' }}>
                        Buscar
                    </Button>
                </InputGroup>
            </div>
            ...
            <div className="bg-white rounded-3 shadow-sm" style={{ border: '1px solid #e9ecef', overflow: 'hidden' }}>
                {carregando ? (
                    ...
                    <Dados
                        consultas={consultas}
                        ExcluirConsulta={handleExcluir}
                        Cadastro={carregarConsultas}
                    />
                )}
            </div>
            ...
            <ModalCadastrar
                ...
                Cadastro={() => { setModalShow(false); carregarConsultas(); }}
            />
        </Container>
    );
}
